...
RPC4::~RPC4() {
    unsigned int i;
    for (i = 0; i < localCallbacks.Size(); i++) { RakNet::OP_DELETE(localCallbacks[i], _FILE_AND_LINE_); }
    ...
    unsigned int j;
    for (j = 0; j < outputList.Size(); j++) { RakNet::OP_DELETE(outputList[j], _FILE_AND_LINE_); }
