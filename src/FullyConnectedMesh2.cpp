...
{
	if (ourFCMGuid==0)
		return mRakPeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS);
...
{
	if (mRakPeerInterface->GetConnectionState(mRakPeerInterface->GetSystemAddressFromGuid(rakNetGuid))!=IS_CONNECTED)
	{
#ifdef DEBUG_FCM2
...
{
	if (rakNetGuid==mRakPeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS))
		return;
...
void FullyConnectedMesh2::AssignOurFCMGuid(void)
{
	...
	randomNumber ^= (unsigned int) (mRakPeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS).g & 0xFFFFFFFF);
	...
	if (AddParticipantInternal(packet->guid,theirFCMGuid))
	{
		...
		for (idx=0; idx < fcm2ParticipantList.Size(); idx++)
		{
			if (packet->guid!=fcm2ParticipantList[idx]->rakNetGuid)
				mRakPeerInterface->Send(&bsOut,HIGH_PRIORITY,RELIABLE_ORDERED,0,fcm2ParticipantList[idx]->rakNetGuid,false);
		}
	}
...
void FullyConnectedMesh2::ConnectToRemoteNewIncomingConnections(Packet *packet)
{
	...
	for (unsigned int i=0; i < count; i++)
	{
		...
		mRakPeerInterface->Connect(str,remoteAddress.GetPort(),connectionPassword.C_String(),(int) connectionPassword.GetLength());
	}
}
