...
void BitStream::EndianSwapBytes(int byteOffset, int length) {
    if (DoEndianSwap()) { ReverseBytesInPlace(data + byteOffset, length); }
}
...
void BitStream::PrintHex(char* out) const {
    BitSize_t i;
    for (i = 0; i < GetNumberOfBytesUsed(); i++) { sprintf(out + i * 3, "%02x ", data[i]); }
}
...
void BitStream::WriteFloat16(float inOutFloat, float floatMin, float floatMax) {
    RakAssert(floatMax > floatMin);
    if (inOutFloat > floatMax + .001) { RakAssert(inOutFloat <= floatMax + .001); }
    if (inOutFloat < floatMin - .001) { RakAssert(inOutFloat >= floatMin - .001); }
