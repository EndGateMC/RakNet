...
TM_TeamMember::~TM_TeamMember() {
    if (world) { world->DereferenceTeamMember(this); }
}
...
void TM_TeamMember::UpdateListsToNoTeam(NoTeamId nti) {
    ...
    for (unsigned int i = 0; i < teams.Size(); i++) { teams[i]->RemoveFromTeamMemberList(this); }
    ...
    if (team == 0) {
        ...
        for (i = 0; i < teamsRequested.Size(); i++) {
            if (teamsRequested[i].requested == team) {
                ...
                if (teamsRequested.Size() == 0) { joinTeamType = JOIN_NO_TEAM; }
                return true;
            }
        }
    }
...
bool TM_Team::DeserializeConstruction(TeamManager* teamManager, BitStream* constructionBitstream) {
    ...
    RakAssert(b);
    if (b) { _world->ReferenceTeam(this, ID, balancingApplies); }
    return b;
}
...
void TM_World::ReferenceTeam(TM_Team* team, NetworkID networkId, bool applyBalancing) {
    ...
    if (applyBalancing && balanceTeamsIsActive) { EnforceTeamBalance(0); }
}
...
void TM_World::Clear(void) {
    for (unsigned int i = 0; i < teams.Size(); i++) { teams[i]->world = 0; }
    for (unsigned int i = 0; i < teamMembers.Size(); i++) { teamMembers[i]->world = 0; }
...
void TeamManager::SetAutoManageConnections(bool autoAdd) {
    ...
    for (unsigned int i = 0; i < worldsList.Size(); i++) { worldsList[i]->SetAutoManageConnections(autoAdd); }
}
...
void TeamManager::OnUpdateTeamsRequestedToAny(Packet* packet, TM_World* world) {
    ...
    bsIn.Read(networkId);
    TM_TeamMember* teamMember = world->GetTeamMemberByNetworkID(networkId);
    if (teamMember) { teamMember->UpdateTeamsRequestedToAny(); }
}
