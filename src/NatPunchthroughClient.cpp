	...
	case ID_NAT_RESPOND_BOUND_ADDRESSES:
		{
			...
			{
				if (natPunchthroughDebugInterface)
					natPunchthroughDebugInterface->OnClientMessage(RakString("INCAPABLE_PORT_STRIDE. My external ID is %s", mRakPeerInterface->GetExternalID(packet->systemAddress).ToString()));
			...
			for (int i=0; i < boundAddressCount && i < MAXIMUM_NUMBER_OF_INTERNAL_IDS; i++)
			{
				...
				if (boundAddresses[i]!=packet->systemAddress)
				{
					...
					mRakPeerInterface->SendOutOfBand((const char*) boundAddresses[i].ToString(false),boundAddresses[i].GetPort(),(const char*) outgoingBs.GetData(),outgoingBs.GetNumberOfBytesUsed());
					break;
				}
			}
		}
...
//	RakAssert(sp.sessionId<100);
	...
	char ipAddressString[32];
	sa.ToString(false, ipAddressString);
	mRakPeerInterface->SendOutOfBand((const char*) ipAddressString,sa.GetPort(),(const char*) oob.GetData(),oob.GetNumberOfBytesUsed());
