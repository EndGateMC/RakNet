...
void StatisticsHistory::Clear(void) {
    for (unsigned int idx = 0; idx < objects.Size(); idx++) { RakNet::OP_DELETE(objects[idx], _FILE_AND_LINE_); }
...
void StatisticsHistory::TimeAndValueQueue::ResizeSampleSet(
    ...
    while (timeBoundary <= endTime) {
        ...
        if (dataCategory == DC_CONTINUOUS) {
            ...
            if (numSamples > 1) { currentSum.val /= numSamples; }
        }
...
void StatisticsHistoryPlugin::OnClosedConnection(
    ...
    if (removeLostConnections) { statistics.RemoveObject(rakNetGUID.g, 0); }
}
