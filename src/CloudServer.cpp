...
void CloudServer::CloudQueryWithAddresses::Serialize(bool writeToBitstream, BitStream* bitStream) {
    ...
    if (writeToBitstream) {
        ...
        for (uint16_t i = 0; i < specificSystems.Size(); i++) { bitStream->Write(specificSystems[i]); }
    } else {
...
void CloudServer::Clear(void) {
    ...
    for (i = 0; i < remoteServers.Size(); i++) { RakNet::OP_DELETE(remoteServers[i], _FILE_AND_LINE_); }
...
void CloudServer::GetServersWithUploadedKeys(
    ...
    unsigned int i, j;
    for (i = 0; i < remoteServers.Size(); i++) { remoteServers[i]->workingFlag = false; }
