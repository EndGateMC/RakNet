...
StartupResult RakPeer::Startup(
    ...
    if (myGuid == UNASSIGNED_RAKNET_GUID) { rnr.SeedMT(GenerateSeedFromGuid()); }
...
//					RakAssert(isRecvFromLoopThreadActive.GetValue()==0);
    ...
    for (i = 0; i < pluginListTS.Size(); i++) { pluginListTS[i]->OnRakPeerStartup(); }

    for (i = 0; i < pluginListNTS.Size(); i++) { pluginListNTS[i]->OnRakPeerStartup(); }
...
void RakPeer::Shutdown(
    ...
    for (i = 0; i < pluginListTS.Size(); i++) { pluginListTS[i]->OnRakPeerShutdown(); }
    for (i = 0; i < pluginListNTS.Size(); i++) { pluginListNTS[i]->OnRakPeerShutdown(); }
...
//	RakNet::TimeMS timeout;
    ...
    for (i = 0; i < socketList.Size(); i++) {
        if (socketList[i]->IsBerkleySocket()) { ((RNS2_Berkley*)socketList[i])->SignalStopRecvPollingThread(); }
    }
#endif
...
#if !defined(__native_client__) && !defined(WINDOWS_STORE_RT)
    for (i = 0; i < socketList.Size(); i++) {
        if (socketList[i]->IsBerkleySocket()) { ((RNS2_Berkley*)socketList[i])->BlockOnStopRecvPollingThread(); }
    }
#endif
...
&recvFromStruct->systemAddress, &recvFromStruct->timeRead); if (recvFromStruct->bytesRead<=0)
    ...
    */
    ...
    for (i = 0; i < pluginListTS.Size(); i++) { pluginListTS[i]->Update(); }
    for (i = 0; i < pluginListNTS.Size(); i++) { pluginListNTS[i]->Update(); }
...
bool RakPeer::SendOutOfBand(
    ...
    if (dataLength > 0) { bitStream.Write(data, dataLength); }
...
ipListFoundIndexStr, incomingRakNetSocket->GetSocketFamily() );
            ...
            if ((int)indexLoopupCheck != (int)assignedIndex) { RakAssert((int)indexLoopupCheck == (int)assignedIndex); }
#endif
...
bool ProcessOfflineNetworkPacket(
    ...
    if (*isOfflineMessage) {
        ...
        // UNCONNECTED MESSAGE Pong with no data.
            ...
            if (serverHasSecurity) { bsIn.Read(cookie); }
                    ...
                    if (thisIPConnectedRecently == false) {
                        ...
                        if (remoteSystem) {
                            ...
                            if (doSecurity) {
                                ...
                                if (doIdentity) { temp.WriteAlignedBytes(ident, sizeof(ident)); }
                            }
#else
...
#ifdef CAT_AUDIT
    printf("AUDIT: RECV ");
    for (int ii = 0; ii < length; ++ii) { printf("%02x", (cat::u8)data[ii]); }
    printf("\n");
#endif
#endif // LIBCAT_SECURITY
...
void RakPeer::DerefAllSockets(void) {
    unsigned int i;
    for (i = 0; i < socketList.Size(); i++) { delete socketList[i]; }
                ...
                if ((unsigned char)(data)[0] == ID_CONNECTION_REQUEST) {
                    ...
                    if (byteSize > sizeof(MessageID) + sizeof(unsigned int) + sizeof(unsigned short)
                        ...
                        if (allowConnection) {
                            ...
                            if (alreadyConnected == false) { PingInternal(systemAddress, true, UNRELIABLE); }
                        } else {
                            // Ignore, already connected
                            rakFree_Ex(data, _FILE_AND_LINE_);
                        }
...
void RakPeer::FillIPList(void) {
    ...
    while (startingIdx < MAXIMUM_NUMBER_OF_INTERNAL_IDS - 1 && ipList[startingIdx] != UNASSIGNED_SYSTEM_ADDRESS) {
        ...
        for (int curIdx = startingIdx + 1;
             ...
             curIdx++) {
            if (ipList[curIdx] < ipList[startingIdx]) { lowestIdx = curIdx; }
        }
