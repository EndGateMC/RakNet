...
#ifdef PRINT_TO_FILE_RELIABLE_ORDERED_TEST
    if (fp == 0 && 0) { fp = fopen("reliableorderedoutput.txt", "wt"); }
#endif
...
void ReliabilityLayer::InitializeVariables(void) {
    ...
    for (int i = 0; i < RNS_PER_SECOND_METRICS_COUNT; i++) { bpsMetrics[i].Reset(_FILE_AND_LINE_); }
}
        ...
        for (i = 0; i < incomingNAKs.ranges.Size(); i++) {
            ...
            for (messageNumber = incomingNAKs.ranges[i].minIndex;
                ...
                while (messageNumberNode) {
                    ...
                    if (internalPacket) {
                        if (internalPacket->nextActionTime != 0) { internalPacket->nextActionTime = timeRead; }
                    }
...
bool ReliabilityLayer::Send(
    ...
    if (numberOfBitsToSend == 0) { return false; }
...
#pragma warning(disable : 4127) // warning C4127: conditional expression is constant
    ...
    if (time > lastBpsClear +
        ...
        for (i = 0; i < RNS_PER_SECOND_METRICS_COUNT; i++) { bpsMetrics[i].ClearExpired1(time); }
