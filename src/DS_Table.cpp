...
void FreeRow(Table::Row* input, int index) {
    ...
    unsigned i;
    for (i = 0; i < input->cells.Size(); i++) { RakNet::OP_DELETE(input->cells[i], _FILE_AND_LINE_); }
...
void Table::RemoveRows(Table* tableContainingRowIDs) {
    ...
    while (cur) {
        for (i = 0; i < (unsigned)cur->size; i++) { rows.Delete(cur->keys[i]); }
        cur = cur->next;
    }
    return;
}
...
void Table::GetCellValueByIndex(unsigned rowIndex, unsigned columnIndex, int* output) {
    ...
    Row* row = GetRowByIndex(rowIndex, 0);
    if (row) { row->cells[columnIndex]->Get(output); }
}
...
void Table::GetCellValueByIndex(unsigned rowIndex, unsigned columnIndex, char* output, int* outputLength) {
    ...
    Row* row = GetRowByIndex(rowIndex, 0);
    if (row) { row->cells[columnIndex]->Get(output, outputLength); }
}
...
void Table::QueryRow(
    ...
    if (inclusionFilterColumnIndices.Size() == 0) {
        ...
        if (pass) { result->AddRowColumns(key, row, columnIndicesToReturn); }
    }
}
...
void Table::SortTable(Table::SortQuery* sortQueries, unsigned numSortQueries, Table::Row** out) {
    ...
    if (anyValid == false) {
        outLength = 0;
        while (cur) {
            for (i = 0; i < (unsigned)cur->size; i++) { out[(outLength)++] = cur->data[i]; }
            cur = cur->next;
        }
        return;
    }
...
Table& Table::operator=(const Table& input) {
    ...
    while (cur) {
        for (i = 0; i < (unsigned int)cur->size; i++) { AddRow(cur->keys[i], cur->data[i]->cells, false); }
