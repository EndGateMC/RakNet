...
ReplicaManager3::~ReplicaManager3() {
    if (autoDestroyConnections) {
        for (unsigned int i = 0; i < worldsList.Size(); i++) { RakAssert(worldsList[i]->connectionList.Size() == 0); }
    }
    Clear(true);
}
...
RakNet::Connection_RM3* ReplicaManager3::PopConnection(RakNetGUID guid, WorldId worldId) {
    ...
    for (index = 0; index < world->connectionList.Size(); index++) {
        if (world->connectionList[index]->GetRakNetGUID() == guid) { return PopConnection(index, worldId); }
    }
    return 0;
}
...
unsigned int ReplicaManager3::ReferenceInternal(RakNet::Replica3* replica3, WorldId worldId) {
    ...
    if (index == (unsigned int)-1) {
        ...
        if (replica3->referenceIndex == (uint32_t)-1) { replica3->referenceIndex = nextReferenceIndex++; }
...
Connection_RM3* ReplicaManager3::GetConnectionBySystemAddress(const SystemAddress& sa, WorldId worldId) const {
    ...
    unsigned int index;
    for (index = 0; index < world->connectionList.Size(); index++) {
        if (world->connectionList[index]->GetSystemAddress() == sa) { return world->connectionList[index]; }
    }
    return 0;
}
...
Connection_RM3* ReplicaManager3::GetConnectionByGUID(RakNetGUID guid, WorldId worldId) const {
    ...
    unsigned int index;
    for (index = 0; index < world->connectionList.Size(); index++) {
        if (world->connectionList[index]->GetRakNetGUID() == guid) { return world->connectionList[index]; }
    }
    return 0;
}
...
void Connection_RM3::OnDownloadFromOtherSystem(Replica3* replica3, ReplicaManager3* replicaManager) {
    ...
    if (constructionMode == QUERY_REPLICA_FOR_CONSTRUCTION
        ...
        unsigned int j;
        for (j = 0; j < queryToConstructReplicaList.Size(); j++) {
            if (queryToConstructReplicaList[j]->replica == replica3) { return; }
        }
...
void Connection_RM3::SendConstruction(
    ...
    for (newListIndex = 0; newListIndex < newObjects.Size(); newListIndex++) {
        ...
        for (int z = 0; z < RM3_NUM_OUTPUT_BITSTREAM_CHANNELS; z++) { sp.outputBitstream[z].ResetWritePointer(); }
...
Replica3::~Replica3() {
    if (replicaManager) { replicaManager->Dereference(this); }
}
