...
void CloudClient::Release(DataStructures::List<CloudKey>& keys, RakNetGUID systemIdentifier) {
    ...
    for (uint16_t i = 0; i < keys.Size(); i++) { keys[i].Serialize(true, &bsOut); }
    SendUnified(&bsOut, HIGH_PRIORITY, RELIABLE_ORDERED, 0, systemIdentifier, false);
}
...
bool CloudClient::Get(
    ...
    for (uint16_t i = 0; i < specificSystems.Size(); i++) { bsOut.Write(specificSystems[i]); }
    SendUnified(&bsOut, HIGH_PRIORITY, RELIABLE_ORDERED, 0, systemIdentifier, false);
    return true;
}
bool CloudClient::Get(
...
void CloudClient::OnSubscriptionNotification(
    ...
    if (deallocateRowAfterReturn) { _allocator->DeallocateRowData(row.data); }
}
void CloudClient::OnSubscriptionNotification(
    bool*           wasUpdated,
    CloudQueryRow*  row,
