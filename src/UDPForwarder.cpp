...
void UDPForwarder::RecvFrom(RakNet::TimeMS curTime, ForwardEntry* forwardEntry) {
    ...
    if (matchConfirmed1 == true || (matchConfirmed2 == false && confirmed1 == false && matchUnconfirmed1 == true)) {
        // Forward to addr2
        if (forwardEntry->addr1Confirmed == UNASSIGNED_SYSTEM_ADDRESS) { forwardEntry->addr1Confirmed = receivedAddr; }
        ...
        // Forward to addr1
        if (forwardEntry->addr2Confirmed == UNASSIGNED_SYSTEM_ADDRESS) { forwardEntry->addr2Confirmed = receivedAddr; }
