...
bool ReadyEvent::IsEventCompleted(int eventId) const {
    bool     objectExists;
    unsigned eventIndex = readyEventNodeList.GetIndexFromKey(eventId, &objectExists);
    if (objectExists) { return IsEventCompletedByIndex(eventIndex); }
    return false;
}
...
bool ReadyEvent::AddToWaitList(int eventId, RakNetGUID guid) {
    ...
    if (guid == UNASSIGNED_RAKNET_GUID) {
        for (i = 0; i < mRakPeerInterface->GetMaximumNumberOfPeers(); i++) {
            ...
            if (firstGuid != UNASSIGNED_RAKNET_GUID) { numAdded += AddToWaitListInternal(eventIndex, firstGuid); }
        }
    } else {
...
bool ReadyEvent::IsInWaitList(int eventId, RakNetGUID guid) {
    ...
    if (objectExists) { return readyEventNodeList[readyIndex]->systemList.HasData(guid); }
    return false;
}
...
unsigned ReadyEvent::GetRemoteWaitListSize(int eventId) const {
    bool     objectExists;
    unsigned readyIndex = readyEventNodeList.GetIndexFromKey(eventId, &objectExists);
    if (objectExists) { return readyEventNodeList[readyIndex]->systemList.Size(); }
    return 0;
}
...
RakNetGUID ReadyEvent::GetFromWaitListAtIndex(int eventId, unsigned index) const {
    bool     objectExists;
    unsigned readyIndex = readyEventNodeList.GetIndexFromKey(eventId, &objectExists);
    if (objectExists) { return readyEventNodeList[readyIndex]->systemList[index].rakNetGuid; }
...
bool ReadyEvent::SetEventByIndex(int eventIndex, bool isReady) {
    ...
    if (IsEventCompletedByIndex(eventIndex)) { PushCompletionPacket(ren->eventId); }
...
void ReadyEvent::Clear(void) {
    unsigned i;
    for (i = 0; i < readyEventNodeList.Size(); i++) { RakNet::OP_DELETE(readyEventNodeList[i], _FILE_AND_LINE_); }
...
void ReadyEvent::UpdateReadyStatus(unsigned eventIndex) {
    ...
    if (ren->eventStatus == ID_READY_EVENT_SET) {
        ...
        if (anyUnset == false) { ren->eventStatus = ID_READY_EVENT_ALL_SET; }
        ...
        if (anyUnset == true) { ren->eventStatus = ID_READY_EVENT_SET; }
    }
