...
bool BPlusTree<KeyType, DataType, order>::FindDeleteRebalance(
    ...
    // Fix underflow:
    if (*underflow) { *underflow = FixUnderflow(branchIndex, cur, rightRootKey, returnAction); }
...
void BPlusTree<KeyType, DataType, order>::ValidateTreeRecursive(Page<KeyType, DataType, order>* cur) {
    ...
    if (cur->children[0]->isLeaf) {
        ...
        for (int i = 0; i < cur->size; i++) { RakAssert(cur->children[i + 1]->keys[0] == cur->keys[i]); }
    } else {
...
void BPlusTree<KeyType, DataType, order>::PrintGraph(void) {
    ...
    if (root) {
        ...
        for (i = 0; i < root->size; i++) { RAKNET_DEBUG_PRINTF("%i ", root->keys[i]); }
        RAKNET_DEBUG_PRINTF(") ");
        RAKNET_DEBUG_PRINTF("\n");
    }
    while (queue.Size()) {
